...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    // JPQL obtain one value
    @Query("select s.firstName from Student s where s.emailId = ?1")
    String getStudentFirstNameByEmailAddress(String emailId);

    // JPQL 2 Parameter
    @Query("select s from Student s where s.emailId = ?1 AND s.lastName = ?2")
    Student getStudentByEmailAddressAndLastName(String emailId, String lastName);
    ...
    int updateStudentNameByEmailId(
            @Param("firstName") String firstName, @Param("emailId") String emailId
    );
}
